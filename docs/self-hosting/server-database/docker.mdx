---
title: Deploying TaamAI CHAT Database with Docker
description: >-
  Learn how to deploy the TaamAI CHAT server database version using Docker on Linux
  and local machines.
tags:
  - TaamAI CHAT
  - Docker
...
<Callout type="info">
This article assumes that you are familiar with the basic principles and processes of deploying the TaamAI CHAT server database version, so it only includes content related to core environment variable configuration. If you are not familiar with the deployment principles of the TaamAI CHAT server database version, please refer to [Deploying Server Database](/docs/self-hosting/server-database) first.
</Callout>

<Callout type="warning">
  Due to the inability to expose `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` using Docker environment variables, you cannot use Clerk as an authentication service when deploying TaamAI CHAT using Docker / Docker Compose.
...
Here is the process for deploying the TaamAI CHAT server database version on a Linux server:
...
<Callout type="info">
The pgvector plugin provides vector search capabilities for Postgres, which is an important component for TaamAI CHAT to implement RAG.
</Callout>
...
S3_BUCKET=TaamAI CHAT
S3_PUBLIC_DOMAIN=https://s3-for-TaamAI CHAT.your-domain.com
...
The data version of TaamAI CHAT also supports direct use on a local Mac/Windows machine.
...
$ docker run -it -d --name lobe-chat-database -p 3210:3210 \
  ...
  -e S3_BUCKET=TaamAI CHAT \
  -e S3_PUBLIC_DOMAIN=https://s3-for-TaamAI CHAT.your-domain.com \
