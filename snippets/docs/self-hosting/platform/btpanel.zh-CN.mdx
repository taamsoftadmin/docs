---
title: 通过 宝塔面板Docker应用商店 部署 TaamAI CHAT
description: 学习如何使用 宝塔面板Docker应用 部署 TaamAI CHAT 服务，包括安装 Docker 容器环境和使用指令一键启动服务。详细说明如何配置环境变量和使用代理地址。
tags:
  - Docker
  - TaamAI CHAT
  - 部署指引
...
3. 安装完成后在`Docker-应用商店-AI/大模型`中找到 `TaamAI CHAT`，点击`安装`
...
## 访问 TaamAI CHAT
- 如果您填写域名，请在浏览器输入您的域名访问，如`http://demo.TaamAI CHAT`，即可访问 `TaamAI CHAT` 页面。
- 请在浏览器地址栏中输入域名访问 `http://<宝塔面板IP>:3210`，即可访问 `TaamAI CHAT` 页面。
![TaamAI CHAT](https://github.com/user-attachments/assets/808f8849-5738-4a60-8ccf-01e300b0dc88)
