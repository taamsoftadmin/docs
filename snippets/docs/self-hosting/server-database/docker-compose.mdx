---
title: Deploying TaamAI CHAT with Docker Compose
description: >-
  Learn how to deploy the TaamAI CHAT service using Docker Compose, including
...
# Deploying TaamAI CHAT Server Database Version with Docker Compose
...
<Callout type="info">
  ...
  TaamAI CHAT server database version (hereinafter referred to as DB version), so it only includes core
  environment variable configurations. If you are not familiar with the deployment principles of
  TaamAI CHAT DB version, please refer to [Deploying with a Server
  Database](/docs/self-hosting/server-database).
</Callout>

<Callout type="warning">
  Due to the inability to expose `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` using Docker environment variables, you cannot use Clerk as a login authentication service when deploying TaamAI CHAT with Docker / Docker Compose.

If you need Clerk as a login authentication service, consider deploying with Vercel or building your own image.

</Callout>

In general, to fully run the TaamAI CHAT database version, you need at least the following four services:

- The TaamAI CHAT database version itself
- A PostgreSQL database with the PGVector plugin
- An object storage service that supports S3 protocol
- An SSO login authentication service supported by TaamAI CHAT

These services can be combined through self-built or online cloud services to meet different levels of deployment needs.

In this article, we provide a Docker Compose configuration entirely based on open-source self-built services. You can use this configuration file directly to start the TaamAI CHAT database version or modify it to suit your needs.
...
To facilitate a quick start, this section uses the `docker-compose/local` directory's `docker-compose.yml` configuration file. The started TaamAI CHAT application runs at `http://localhost:3210` and can be run locally.
...
At this point, you have successfully deployed the TaamAI CHAT database version, and you can access your TaamAI CHAT service at `http://localhost:3210`.
...
The following assumes that the machine IP running the TaamAI CHAT service on the LAN/public network is `your_server_ip`.
...
- `lobe.example.com`: Your TaamAI CHAT service domain, which needs to reverse proxy to the TaamAI CHAT service port, defaulting to `3210`.
...
The configuration files include both `.env` and `docker-compose.yml`, where the `.env` file is used to configure the environment variables for TaamAI CHAT, and the `docker-compose.yml` file is used to configure the Postgres, MinIO, and Logto services.
...
<Callout type="warning">
  Please remember to configure the CORS cross-domain settings for the corresponding login authentication service provider to ensure TaamAI CHAT can access the authentication service properly.
...
8. Restart the TaamAI CHAT service:
...
<Callout type="warning">
  ...
  TaamAI CHAT with your administrator account, as this will only result in an error.
</Callout>
...
<Callout type="warning">
  Please remember to configure the CORS cross-domain settings for the corresponding S3 service provider to ensure TaamAI CHAT can access the S3 service properly.
...
5. Restart the TaamAI CHAT service:

   ```sh
   docker compose up -d
   ```

At this point, you have successfully deployed the TaamAI CHAT database version, and you can access your TaamAI CHAT service at `https://lobe.example.com`.
...
```sh
# Required: TaamAI CHAT domain for tRPC calls
...
# For supported providers, see: https://taam.cloud/docs/self-hosting/advanced/auth#next-auth
...
# For other providers, see: https://taam.cloud/docs/self-hosting/environment-variables/auth
...
# See: https://taam.cloud/docs/self-hosting/advanced/s3#s-3-enable-path-style
S3_ENABLE_PATH_STYLE=1

# Other basic environment variables (as needed)
# See: https://taam.cloud/docs/self-hosting/environment-variables/basic
...
services:
  ...
  minio:
    ...
    environment:
      ...
      - 'MINIO_API_CORS_ALLOW_ORIGIN=https://lobe.example.com' # Your TaamAI CHAT's domain name.
    restart: always
    command: >
