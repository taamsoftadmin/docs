---
title: 在 Zeabur 上部署 TaamAI CHAT
description: 按照指南准备 OpenAI API Key 并点击按钮进行部署。在部署完成后，即可开始使用 TaamAI CHAT 并选择是否绑定自定义域名。
tags:
  - Zeabur
  - TaamAI CHAT
  - OpenAI API Key
...
# 使用 Zeabur 部署 TaamAI CHAT 数据库版

<Callout type="info">
  本文假设你已经熟悉 TaamAI CHAT
  服务器数据库版的部署基本原理和流程，因此只包含与核心环境变量配置相关的内容。如果你对 TaamAI CHAT
...
- MinIO 作为对象存储
- TaamAI CHAT Database 的实例

## 在 Zeabur 上部署

这里是在 Zeabur 上部署 TaamAI CHAT 服务器数据库版的流程：

<Steps>

### 前往 Zeabur 上的模板页面

前往 [Zeabur 上的 TaamAI CHAT 数据库模板页面](https://zeabur.com/templates/RRSPSD) 并点击 "Deploy" 按钮。
...
- TaamAI CHAT Domain: 一个免费的 `.zeabur.app` 后缀的域名。
...
在你填写完所有必要的环境变量后，选择一个你想要部署 TaamAI CHAT 数据库的区域并点击“部署”按钮。
...
使用你刚绑定的域名来访问你的 Logto 控制台，创建一个新项目以获得对应的客户端 ID 与密钥，将它们填入你的 TaamAI CHAT 服务的变量中。
...
按下 `TaamAI CHAT-Database` 你会看到你刚刚创建的公共域名，点击它以访问你的 TaamAI CHAT 数据库。
