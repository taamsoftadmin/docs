---
title: TaamAI CHAT Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for TaamAI CHAT Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest TaamAI CHAT image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running TaamAI CHAT Container

Assuming the TaamAI CHAT container is named `lobe-chat`, use the following commands to stop and remove the currently running TaamAI CHAT container:
...
### Pull the Latest TaamAI CHAT Image

Use the following command to pull the latest Docker image for TaamAI CHAT:
...
Redeploy the TaamAI CHAT container using the newly pulled image:
...
No need to worry, you won't. All of TaamAI CHAT's chat records are stored in your local browser. Therefore, when redeploying TaamAI CHAT using Docker, your chat records will not be lost.
