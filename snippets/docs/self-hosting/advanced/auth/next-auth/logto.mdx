---
title: Configuring Logto Authentication Service in TaamAI CHAT
description: >-
  Learn how to configure Logto authentication service in TaamAI CHAT, including
...
<Callout type={'tip'}>
  ...
  with the TaamAI CHAT database version. In this case, TaamAI CHAT can share the same Postgres instance
  with it.
</Callout>

## Logto Configuration Process

The following assumes your TaamAI CHAT database version domain is `https://lobe.example.com`.
...
<Steps>
  ...
  Set the obtained `Client ID` and `Client Secret` as `AUTH_LOGTO_ID` and `AUTH_LOGTO_SECRET` in the TaamAI CHAT environment variables.

  Configure `AUTH_LOGTO_ISSUER` in the TaamAI CHAT environment variables as follows:
  ...
  | `NEXT_AUTH_SSO_PROVIDERS` | Required | Select the single sign-on provider for TaamAI CHAT. For Logto, enter `logto`.                                                                                                             |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate via Logto and use TaamAI CHAT.
</Callout>
